...
因子计算 + 看板生成脚本
读取 data/ 里的价格和基本面数据
输出完整看板到 docs/index.html (GitHub Pages 默认读取这个路径)
"""
...
ranking["全市场排名"] = range(1, len(ranking)+1)
print(f"排名完成: {len(ranking)} 只")

# --- 前30组合 ---
top30    = ranking.head(30).index.tolist()
weights  = score_weighted_target_weights(ranking["composite_score"], top30, DEFAULT_CONFIG["max_weight"])
result   = ranking.loc[top30].copy()
result["target_weight"] = weights
result["market_cap_B"]  = (filtered.loc[top30, "market_cap"] / 1e9).round(1)
...
def calc_macd(series):
    ...
    hist = (dif - dea) * 2
    df = pd.DataFrame({"c":w,"dif":dif,"dea":dea,"h":hist}).loc[display_start:]
...
data_js = (
    ...
    "window.SECTORS=" + json.dumps({"sectors": sectors_data, "asOf": as_of.strftime("%Y-%m-%d")}, ensure_ascii=False, separators=(',',':')) + ";\n"
)
with open("docs/data.js", "w") as f:
    f.write(data_js)
print(f"data.js 大小: {len(data_js)//1024} KB")

# --- 复制看板HTML到 docs/index.html ---
# 从 templates/dashboard.html 读取模板
...
print(f"=== 看板构建完成 ===")
print(f"访问地址: https://gaojianz.github.io/quant-dashboard/")
